...
DATA_PAC <- gapminder::gapminder %>%
  ...
  mutate(
    ...
    Subregion = case_when(
      ...
      País %in% c("Cuba", "Dominican Republic", "Haiti", "Jamaica", "Trinidad and Tobago") ~ "El Caribe",
      TRUE ~ "Otros"
    ),
    iso_code = case_when(
      ...
      País == "United States" ~ "USA", País == "Uruguay" ~ "URY", País == "Venezuela" ~ "VEN",
      TRUE ~ NA_character_
    )
  )
...
server <- function(input, output, session) {
  ...
  output$vbox_vida <- renderValueBox({
    df <- datos_filtrados()
    val <- if(nrow(df) > 0) round(mean(df$Esperanza_de_vida, na.rm=TRUE), 1) else 0
  ...
  output$vbox_pop <- renderValueBox({
    ...
    valueBox(scales::comma(val), "Población Total", icon = icon("users"), color = "blue")
  })
